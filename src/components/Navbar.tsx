    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a href="#register" className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-full text-sm font-semibold transition-all">
            사전 등록하기
          </a>
        </nav>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#050505] border-b border-white/10 p-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="text-lg font-medium text-gray-300 hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
              {link.name}
            </a>
          ))}
          <a href="#register" className="bg-purple-600 text-white text-center px-6 py-3 rounded-full text-base font-semibold mt-4" onClick={() => setIsMobileMenuOpen(false)}>
            사전 등록하기
          </a>
        </div>
      )}
